...
export const STAGE_ORDER = ['city', 'colony'];
...
let colonyGltf = null;      // 一度読んだら使い回す
let colonyBoxes = null;

async function fetchColony() {
  if (colonyGltf && colonyBoxes) return { scene: colonyGltf, boxes: colonyBoxes };
  if (!dracoLoader) {
  ...
  const [gltf, boxes] = await Promise.all([
    loader.loadAsync(asset('models/colony.glb')),
    fetch(asset('models/colony_boxes.json')).then((r) => r.json()),
  ]);
  colonyGltf = gltf.scene;
  colonyGltf.traverse((o) => { o.userData.shared = true; });
  colonyBoxes = boxes;
  return { scene: colonyGltf, boxes };
}

// GLB のマテリアルはゲーム側で詰める。書き出し直さずに調整できるようにしておく。
//   mul  … ベースカラーの倍率（Blender で塗り直さずに明暗を決める）
//   emis … emissiveIntensity。ブルームの閾値は 0.86。採光窓は画面を大きく占めるので
//           ここを上げすぎると画面全体が白く飛ぶ
const COLONY_MAT = {
...
async function buildColony(scene) {
  const { scene: model, boxes } = await fetchColony();
  ...
  scene.fog = new THREE.Fog('#1b2c4d', 340, 1900);
  ...
  model.traverse((o) => {
    ...
    o.castShadow = cast;
    // プレイ範囲の建物だけカメラの当たり判定に使う（遠景まで拾うと重い）
    if (o.name.startsWith('CityBlocks')) buildings.push(o);
  });

  // 平地の床。GLB の地面と同一平面だが、影を受ける面が要る
  const ground = new THREE.Mesh(
    new THREE.CircleGeometry(ARENA_R + 26, 64),
    new THREE.MeshStandardMaterial({ map: groundTexture(), color: 0x6b7793, roughness: 0.92, metalness: 0.1 })
  );
  ground.rotation.x = -Math.PI / 2;
  ...
  return { grp, ground, buildings, boxes, lights, far: 2200, envIntensity: 0.4 };
}
...
export async function buildStage(scene, renderer, id) {
  const built = id === 'colony' ? await buildColony(scene) : buildCity(scene);
...
export function prefetchStage(id) {
  if (id === 'colony') fetchColony().catch(() => {});
}
